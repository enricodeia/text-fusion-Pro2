...
const quickPresetSelect = document.getElementById("quick-preset");
...
function applyQuickPreset(preset) {
  ...
  if (animationSelect) animationSelect.value = config.animation;
  currentSplitType = config.splitType;
  if (speedSlider) speedSlider.value = config.speed;
  if (delaySlider) delaySlider.value = config.delay;
  if (easeSelect) easeSelect.value = config.ease;
  if (fontSizeInput) fontSizeInput.value = config.fontSize;
  if (textWeightSelect) textWeightSelect.value = config.fontWeight || '700';
  if (letterSpacingInput) letterSpacingInput.value = config.letterSpacing || 0;
  if (textTransformSelect) textTransformSelect.value = config.textTransform || 'uppercase';
  
  // Apply font if set
  if (config.fontFamily && fontSelect) { fontSelect.value = config.fontFamily; }
  
  // Apply text shadow if set
  if (config.textShadow !== undefined && textShadowInput) { textShadowInput.value = config.textShadow; }
  
  // Apply toggles
  if (use3dInput) use3dInput.checked = config.use3d || false;
  if (useMultiColorInput) useMultiColorInput.checked = config.useMultiColor || false;
  if (useMaskInput) useMaskInput.checked = config.useMask || false;
  
  // Apply mask settings if enabled
  if (config.useMask && maskControls && maskTypeSelect) {
    maskControls.style.display = "block";
    maskTypeSelect.value = config.maskType || "circle";
    if (customMaskControls)
      customMaskControls.style.display = maskTypeSelect.value === "custom" ? "block" : "none";
  } else if (maskControls) {
    maskControls.style.display = "none";
  ...
  // Apply repeat iterations if set
  if (config.iterations !== undefined && iterationsInput) { iterationsInput.value = config.iterations; }
  
  // Apply stagger order if set
  if (config.staggerOrder && staggerOrderSelect) { staggerOrderSelect.value = config.staggerOrder; }
